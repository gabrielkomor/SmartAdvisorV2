"""
The functions in this file are responsible for calculating the values of investment
signals generated by expert systems using fuzzy logic.
"""
...
def calculate_signals_rsi(
    ...
    :param signals: transaction signals.
    :return: nothing, modifies existing array.
    """
    rsi_values = np.array([22, 30, 34, 46, 54, 66, 70, 78], dtype=np.int16)
    signals[0, num] = signals[2, num] = signals[1, num] = 0

    if rsi <= rsi_values[0]:
        signals[2, num], signals[0, num], signals[1, num] = 1, 0, 0

    elif rsi_values[0] < rsi < rsi_values[1]:
        signals[0, num] = (rsi - rsi_values[0]) / (rsi_values[1] - rsi_values[0])
        signals[2, num] = (rsi_values[1] - rsi) / (rsi_values[1] - rsi_values[0])

    elif rsi_values[1] <= rsi <= rsi_values[2]:
        signals[0, num], signals[1, num], signals[2, num] = 1, 0, 0

    elif rsi_values[2] < rsi < rsi_values[3]:
        signals[0, num] = (rsi_values[3] - rsi) / (rsi_values[3] - rsi_values[2])
        signals[2, num] = (rsi - rsi_values[2]) / (rsi_values[3] - rsi_values[2])

    elif rsi_values[3] <= rsi <= rsi_values[4]:
        signals[2, num], signals[0, num], signals[1, num] = 1, 0, 0

    elif rsi_values[4] < rsi < rsi_values[5]:
        signals[2, num] = (rsi_values[5] - rsi) / (rsi_values[5] - rsi_values[4])
        signals[1, num] = (rsi - rsi_values[4]) / (rsi_values[5] - rsi_values[4])

    elif rsi_values[5] <= rsi <= rsi_values[6]:
        signals[1, num], signals[2, num], signals[0, num] = 1, 0, 0

    elif rsi_values[6] < rsi < rsi_values[7]:
        signals[2, num] = (rsi - rsi_values[6]) / (rsi_values[7] - rsi_values[6])
        signals[1, num] = (rsi_values[7] - rsi) / (rsi_values[7] - rsi_values[6])

    elif rsi >= rsi_values[7]:
        signals[2, num], signals[0, num], signals[1, num] = 1, 0, 0

    signals[0, num], signals[1, num], signals[2, num] = np.round(
        [signals[0, num], signals[1, num], signals[2, num]], 5
    )
...
def calculate_signals_sma(
    ...
    if sma_d <= sma1:
        ...
        signals[1, num] = (sma2 - sma_d) / (sma2 - sma1)
    ...
    if sma_d >= sma4:
        ...
        signals[0, num] = (sma_d - sma3) / (sma4 - sma3)
    ...
    if sma2 <= sma_d <= sma3:
        ...
        signals[2, num] = (sma_d - sma1) / (sma2 - sma1)
    elif sma3 < sma_d < sma4:
        signals[2, num] = (sma4 - sma_d) / (sma4 - sma3)
...
def calculate_signals_bollinger_bands(
    ...
    if current_price <= x1:
        ...
        signals[0, num] = (x2 - current_price) / (x2 - x1)

    if current_price >= x4:
        signals[1, num], signals[0, num], signals[2, num] = 1, 0, 0
    elif x3 < current_price < x4:
        signals[1, num] = (current_price - x3) / (x4 - x3)

    if x2 <= current_price <= x3:
        signals[2, num], signals[1, num], signals[0, num] = 1, 0, 0
    elif x1 < current_price < x2:
        signals[2, num] = (current_price - x1) / (x2 - x1)
    elif x3 < current_price < x4:
        signals[2, num] = (x4 - current_price) / (x4 - x3)
...
def calculate_signals_macd(
    ...
    if current_price <= x1:
        ...
        signals[1, num] = (x2 - current_price) / (x2 - x1)

    if current_price >= x4:
        signals[0, num], signals[2, num], signals[1, num] = 1, 0, 0
    elif x3 < current_price < x4:
        signals[0, num] = (current_price - x3) / (x4 - x3)
...
def calculate_signals_adx(
    ...
    if adx_value > 20:
        ...
        if adx_value <= x1:
            ...
            signals[1, num] = (x2 - adx_value) / (x2 - x1)

        if adx_value >= x4:
            signals[0, num], signals[2, num], signals[1, num] = 1, 0, 0
        elif x3 < adx_value < x4:
            signals[0, num] = (adx_value - x3) / (x4 - x3)

        if x2 <= adx_value <= x3:
            signals[2, num], signals[1, num], signals[0, num] = 1, 0, 0
        elif x1 < adx_value < x2:
            signals[2, num] = (adx_value - x1) / (x2 - x1)
        elif x3 < adx_value < x4:
            signals[2, num] = (x4 - adx_value) / (x4 - x3)
    else:
        signals[2, num], signals[1, num], signals[0, num] = 1, 0, 0
...
def calculate_signals_volume(
    ...
    if min_value <= current_value <= max_value:
        signals[0, num] = signals[2, num] = signals[1, num] = 0.33
